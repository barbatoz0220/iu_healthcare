...
function deleteDoctor(index) {
  alertify
    .confirm(
      ...
      function () {
        fetch(`/admin/doctor-list/delete/${index}`).then(response => {
          return response.text().then(text => {
            document.getElementById("container").innerHTML = text;
...
function submitAddForm() {
  if (
    ...
    alertify
      .alert("Please fill in all the fields!", function () {
        alertify.message("OK");
      })
      .set({ title: "Failed" })
      .set("closable", false);
    ...
    alertify
      .confirm(
        "Do you want to insert the doctor?",
        function () {
          var formData = {
...
function returnPageList() {
  fetch("/admin/doctor-list/pagination?page=1").then(response => {
    return response.text().then(text => {
      document.getElementById("container").innerHTML = text;
    });
  });
